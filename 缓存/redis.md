...
## 字典结构Hash
+ Redis的Hash结构用于保存KV类型的数据，但是它并不是直接使用Hash存储数据的，在创建时，默认采用ZipList(压缩列表)的方式实现，可以节省空间。当列表中的节点数量大于512个的时候才会改为Hash表实现
...
# 解决缓存不一致
+ 缓存不一致是指缓存数据与数据库数据不一致，主要发生在数据更新的情况，数据的更新一般是先更新数据库后更新缓存，当线程A,B同时对一个数据进行更新，A先将数据库更新为100,B然后将数据库更新为200,此时数据库是200,缓存也应该更新为200,但是可能因为网络原因，A却在B后面更新了缓存，就导致缓存变成了100,此时缓存就不一致了
+ 解决缓存不一致的方案
	- 使用分布式事务，将数据库更新和缓存更新作为一个事务，此方案实现复杂，而且影响系统效率，但是安全性高
	- 放弃更新缓存，而是删除缓存，这种方案更简单，但是会减小缓存命中率，而且在数据库更新完毕到删除缓存这段时间，整个系统有一小段时间的不一致。注意此方案必须先更新数据库后删除缓存
	- 更新操作由单线程完成，实现简单，安全，但是失去了并发性
